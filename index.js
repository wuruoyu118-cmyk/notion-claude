...
return [rt.map](http://rt.map)((x) => x?.plain_text || "").join("");
...
const text = [allBlocks.map](http://allBlocks.map)(getTxt).filter((t) => t.trim()).join("nn");
...
const timePattern = /(d{4}[-./年]d{1,2}[-./月]d{1,2}日?)|(d{1,2}月d{1,2}日)/;
...
e?.response?.data ? JSON.stringify([e.response.data](http://e.response.data)) :
...
[app.post](http://app.post)("/messages", async (req, res) => {
